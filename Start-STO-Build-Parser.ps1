...
try {
    ...
    if ($existing.Content -match '<title>STO Shakedown</title>') {
...
try {
    ...
    for ($attempt = 0; $attempt -lt 40; $attempt++) {
        ...
        try {
            ...
            if ($response.Content -match '<title>STO Shakedown</title>') { $ready = $true; break }
        } catch { }
        Start-Sleep -Milliseconds 250
    }
    if (-not $ready) { throw 'The app did not become ready.' }
    Start-Process $appUrl
    Write-Host 'STO Shakedown is running. Keep this window open; press Ctrl+C to stop.'
