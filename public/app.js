...
function buildOptions(){const builds=state.builds.map(b=>({value:b.id,label:`${b.ship} / ${b.name}`}));for(const id of ['save-build','baseline','candidate'])options(id,builds,'Choose a version');const players=[...new Map(state.runs.map(r=>[r.player.id,r.player])).values()];options('compare-player',players.map(p=>({value:p.id,label:`${p.name} · ${p.id}`})),'Choose a player');options('compare-context',[...new Set(state.runs.map(r=>r.context))].map(c=>({value:c,label:c})),'Choose encounter / difficulty');}
function renderBuilds(){$('build-list').innerHTML=state.builds.length?state.builds.map(b=>`<article class="panel build-card"><div class="eyebrow">${esc(b.ship)}</div><h2>${esc(b.name)}</h2><pre>${esc(b.notes||'No equipment notes yet.')}</pre><p>${state.runs.filter(r=>r.buildId===b.id).length} saved runs</p>${state.runs.filter(r=>r.buildId===b.id).map(r=>`<p class="muted">${esc(r.stamp)} · ${esc(r.context)}<br>${esc(r.player.name)} · ${num(r.player.dps)} DPS</p>`).join('')}</article>`).join(''):'<div class="panel"><h2>No build versions yet.</h2><p>Create your baseline above. Give each equipment change its own version.</p></div>';}
async function refresh(){state=await api('state');$('folder-path').value=state.folder;$('folder-summary').textContent=state.folder||'No folder selected yet. Choose your own STO log location to begin.';options('log',state.logs.map(l=>({value:l.name,label:`${l.name} · ${(l.size/1e6).toFixed(1)} MB`})),'Choose a combat log');buildOptions();renderBuilds();if(state.folderError)notice(state.folderError,true);}
