import {patrols,difficulties,battleTypes} from './patrols.js';
...
function patrolForm(prefix,value={}) {
 ...
 const html=`<label>Battle type<select id="${prefix}-battle-type">${battleTypes.map(t=>`<option ${t===(value.battleType??'Patrol')?'selected':''}>${t}</option>`).join('')}</select></label><label id="${prefix}-name-label">Mission name<input id="${prefix}-name" maxlength="200" value="${esc(value.missionName||'')}" placeholder="Enter the TFO or DSE name"><small>Use the same mission name when comparing runs.</small></label><label id="${prefix}-category-label">Patrol category<select id="${prefix}-category">${[...new Set(patrols.map(p=>p.category))].map(c=>`<option ${c===chosen?.category?'selected':''}>${esc(c)}</option>`).join('')}</select></label><label id="${prefix}-mission-label">Space patrol<select id="${prefix}-mission"></select></label><label>Difficulty<select id="${prefix}-difficulty"><option value="">Choose difficulty</option>${difficulties.map(d=>`<option ${d===value.difficulty?'selected':''}>${d}</option>`).join('')}</select></label><label>Party<select id="${prefix}-party"><option ${value.party!=='Group'?'selected':''}>Solo</option><option ${value.party==='Group'?'selected':''}>Group</option></select></label><p id="${prefix}-patrol-help" class="muted">* Enemy groups can vary between runs and affect results. Hard / Normal in a patrol name is separate from difficulty.</p>`;
 return {html,bind(){const changeType=()=>{const patrol=$(prefix+'-battle-type').value==='Patrol';for(const suffix of ['-category-label','-mission-label','-patrol-help'])$(prefix+suffix).hidden=!patrol;$(prefix+'-name-label').hidden=patrol;};$(prefix+'-battle-type').onchange=changeType;changeType();const update=()=>options(prefix+'-mission',patrols.filter(p=>p.category===$(prefix+'-category').value).map(p=>({value:p.id,label:p.name+(p.random?' *':'')})),'Choose patrol');$(prefix+'-category').onchange=update;update();if(chosen)$(prefix+'-mission').value=chosen.id;}};
}
function patrolValue(prefix){return {battleType:$(prefix+'-battle-type').value,missionName:$(prefix+'-name').value,patrolId:$(prefix+'-mission').value,difficulty:$(prefix+'-difficulty').value,party:$(prefix+'-party').value};}
