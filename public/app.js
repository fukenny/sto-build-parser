...
function page(id){window.scrollTo(0,0);document.querySelector("main").scrollTo(0,0);document.querySelectorAll('.page').forEach(p=>p.hidden=p.id!==id);document.querySelectorAll('.nav').forEach(n=>n.classList.toggle('active',n.dataset.page===(id==='builds'?'shipyard':id)));}
...
function renderBuildCards(){$('build-list').innerHTML=shipBuilds().length?shipBuilds().map(b=>`<article class="panel build-card"><div class="eyebrow">${esc(b.ship)} / ${esc(state.loadouts?.find(l=>l.id===b.loadoutId)?.name)}</div><h2>${esc(b.name)}</h2><pre>${esc(b.notes||'No equipment notes yet.')}</pre><p>${state.runs.filter(r=>r.buildId===b.id).length} saved runs</p>${state.runs.filter(r=>r.buildId===b.id).map(r=>`<p class="muted">${esc(r.stamp)} · ${esc(r.context)}${r.patrolId?"":" · Needs patrol confirmation"}<br>${esc(r.player.name)} · ${num(r.player.dps)} DPS <button data-edit-run="${esc(r.id)}">Edit label / version</button></p>`).join('')}</article>`).join(''):'<div class="panel"><h2>No loadouts yet.</h2><p>Add a loadout above to create its Baseline variation.</p></div>';}
async function refresh(){state=await api('state');if(!state.profiles.some(p=>p.id===activeShip))activeShip=state.profiles[0]?.id||'';options('workspace-ship',state.profiles.map(p=>({value:p.id,label:p.name})),'Choose a ship');$('workspace-ship').value=activeShip;renderShips();$('folder-path').value=state.folder;$('folder-summary').textContent=state.folder||'No folder selected yet. Choose your own STO log location to begin.';options('log',state.logs.map(l=>({value:l.name,label:`${l.name} · ${(l.size/1e6).toFixed(1)} MB`})),'Choose a combat log');buildOptions();renderBuilds();if(state.folderError)notice(state.folderError,true);}
...
$('workspace-ship').onchange=()=>{chooseShip($('workspace-ship').value);if(!$('shipyard').hidden)page('builds');};
