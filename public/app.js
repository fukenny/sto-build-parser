...
function buildOptions(){const builds=state.builds.map(b=>({value:b.id,label:buildLabel(b)}));for(const id of ['save-build','baseline','candidate'])options(id,builds,'Choose a version');const players=[...new Map(state.runs.map(r=>[r.player.id,r.player])).values()];options('compare-player',players.map(p=>({value:p.id,label:`${p.name} · ${p.id}`})),'Choose a player');options('compare-context',[...new Set(state.runs.map(r=>r.context))].map(c=>({value:c,label:c})),'Choose encounter / difficulty');}
function renderBuilds(){$('build-list').innerHTML=state.builds.length?state.builds.map(b=>`<article class="panel build-card"><div class="eyebrow">${esc(b.ship)} / ${esc(state.loadouts?.find(l=>l.id===b.loadoutId)?.name)}</div><h2>${esc(b.name)}</h2><pre>${esc(b.notes||'No equipment notes yet.')}</pre><p>${state.runs.filter(r=>r.buildId===b.id).length} saved runs</p>${state.runs.filter(r=>r.buildId===b.id).map(r=>`<p class="muted">${esc(r.stamp)} · ${esc(r.context)}<br>${esc(r.player.name)} · ${num(r.player.dps)} DPS <button data-edit-run="${esc(r.id)}">Edit label / version</button></p>`).join('')}</article>`).join(''):'<div class="panel"><h2>No variations yet.</h2><p>Create your baseline above. Give each equipment change its own version.</p></div>';}
async function refresh(){state=await api('state');$('folder-path').value=state.folder;$('folder-summary').textContent=state.folder||'No folder selected yet. Choose your own STO log location to begin.';options('log',state.logs.map(l=>({value:l.name,label:`${l.name} · ${(l.size/1e6).toFixed(1)} MB`})),'Choose a combat log');$('ship-names').innerHTML=(state.profiles||[]).map(p=>`<option value="${esc(p.name)}"></option>`).join('');$('loadout-names').innerHTML=(state.loadouts||[]).map(l=>`<option value="${esc(l.name)}"></option>`).join('');buildOptions();$('context-labels').innerHTML=[...new Set(state.runs.map(r=>r.context))].map(c=>`<option value="${esc(c)}"></option>`).join('');renderBuilds();if(state.folderError)notice(state.folderError,true);}
...
$('build-list').addEventListener('click',async event=>{const id=event.target.dataset.editRun;if(!id)return;const r=state.runs.find(r=>r.id===id);const card=event.target.closest('article');if(card.querySelector('.run-editor'))return;const editor=document.createElement('div');editor.className='run-editor';editor.innerHTML=`<label>Mission / difficulty<input class="edit-context" list="context-labels" value="${esc(r.context)}" maxlength="300"></label><label>Attach to version<select class="edit-build">${state.builds.map(b=>`<option value="${esc(b.id)}" ${b.id===r.buildId?'selected':''}>${esc(b.ship)} / ${esc(b.name)}</option>`).join('')}</select></label><button class="apply-edit">Save correction</button><button class="cancel-edit">Cancel</button>`;card.append(editor);editor.querySelector('.cancel-edit').onclick=()=>editor.remove();editor.querySelector('.apply-edit').onclick=async()=>{try{await api('run/edit',{id,context:editor.querySelector('.edit-context').value,buildId:editor.querySelector('.edit-build').value});await refresh();notice('Saved run updated. A backup of the previous workspace was kept.');}catch(e){notice(e.message,true);}};});
