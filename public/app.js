...
bindDamageRows(document.querySelector('main'));
...
function renderSurvival(player, encounter) {
  ...
  $('damage-sources').innerHTML = sourceRows(s.sources);
  $('healing-sources').innerHTML = sourceRows(s.healers,true);
...
function contributionTables(result){
 const table=(title,rows)=>`<h2>${title}</h2><div class="table-wrap"><table><thead><tr><th>Recorded item</th><th>Source</th><th>Baseline DPS</th><th>Candidate DPS</th><th>DPS change</th></tr></thead><tbody>${comparisonRows(rows,result.baseline.count,result.candidate.count)}</tbody></table></div>`;
...
function renderCombatDetails(player) {
 ...
 $('briefing-rates').innerHTML=known.length?metric('HIT ACCURACY',rate(total.hits,total.hits+total.misses),`${num(total.hits)} hits · ${num(total.misses)} misses`)+metric('CRITICAL HIT RATE',rate(total.crits,total.hits),`${num(total.crits)} critical hits`)+metric('FLANK RATE',rate(total.flanks,total.hits),`${num(total.flanks)} flanking hits`):'<p>Reanalyze this log to calculate combat details.</p>';
}
