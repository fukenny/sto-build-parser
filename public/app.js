...
function renderPlayer(){const {encounter:e,player:p}=selection();$('results').hidden=!p;$('empty').hidden=!!p;if(!p)return;renderSurvival(p,e);renderCombatDetails(p);$('metrics').innerHTML=metric(e.scope==='full-log'?'FULL-LOG DPS':'ENCOUNTER DPS',num(p.dps),`${e.duration.toFixed(1)}s encounter · hull + shield`)+metric('TOTAL DAMAGE',compact(p.total),`${compact(p.hull)} hull / ${compact(p.shield)} shield`)+metric('PETS & SUMMONS',compact(p.pets),`${p.total?(100*p.pets/p.total).toFixed(1):0}% of outgoing damage`)+metric('DAMAGE TAKEN',compact(p.incoming),`${compact(p.healing)} logged outgoing healing`);renderBriefing(p,e);$('abilities').innerHTML=p.abilities.map(a=>{const share=p.total?100*a.total/p.total:0;return `<tr><td>${esc(a.name)}</td><td class="${a.pet?'pet':''}">${esc(a.source)}</td><td>${num(a.total)}</td><td>${share.toFixed(1)}%<div class="share"><i style="width:${share.toFixed(2)}%"></i></div></td><td>${num(a.total/e.duration)}</td><td>${num(a.events)}</td></tr>`;}).join('');}
...
function renderSurvival(player, encounter) {
  ...
  const active = bins.filter(d=>d>0).sort((a,b)=>a-b);
  const tail = Math.max(1,Math.ceil(active.length*.1));
  ...
  const bandLabel = {zero:'No damage',normal:'Typical range',high:'Highest 10% (including ties)',low:'Lowest 10% (including ties)'};
  $('pressure-legend').textContent=active.length<10?'Fewer than 10 nonzero bars: percentile highlighting is off.':low===high?'Damage values are tied: percentile highlighting is off.':'Red: highest 10% · Yellow: lowest 10% · Lavender: middle range · Gray: no damage. Ranked within this run’s nonzero bars; ties included. These are relative levels, not danger thresholds.';
  ...
  $('pressure-chart').innerHTML = bins.map((damage,i)=>`<div class="pressure-bar pressure-${band(damage)}" tabindex="0" style="height:${Math.max(2,100*damage/peak)}%" title="+${i*width}–${(i+1)*width}s: ${num(damage)} damage · ${bandLabel[band(damage)]}" aria-label="${i*width} to ${(i+1)*width} seconds: ${num(damage)} damage · ${bandLabel[band(damage)]}"></div>`).join('');
...
function renderCombatDetails(player) {
 const rate=(hits,total)=>total?`${(100*hits/total).toFixed(1)}%`:'—';
 const known=player.abilities.filter(a=>Number.isFinite(a.misses));
 const total=known.reduce((s,a)=>({hits:s.hits+a.hullHits,misses:s.misses+a.misses,crits:s.crits+a.criticalHits,flanks:s.flanks+a.flankHits}),{hits:0,misses:0,crits:0,flanks:0});
 $('combat-detail-metrics').innerHTML=known.length?metric('HIT ACCURACY',rate(total.hits,total.hits+total.misses),`${num(total.hits)} hits · ${num(total.misses)} misses`)+metric('CRITICAL HIT RATE',rate(total.crits,total.hits),`${num(total.crits)} critical hits`)+metric('FLANK RATE',rate(total.flanks,total.hits),`${num(total.flanks)} flanking hits`):'<p>Reanalyze this log to calculate combat details.</p>';
 $('combat-detail-rows').innerHTML=player.abilities.map(a=>!Number.isFinite(a.misses)?`<tr><td>${esc(a.name)}</td><td>${esc(a.source)}</td><td colspan="10">Not recorded — reanalyze this log.</td></tr>`:`<tr><td>${esc(a.name)}</td><td>${esc(a.source)}</td><td>${num(a.hullHits)}</td><td>${num(a.misses)}</td><td>${rate(a.hullHits,a.hullHits+a.misses)}</td><td>${num(a.criticalHits)}</td><td>${rate(a.criticalHits,a.hullHits)}</td><td>${num(a.criticalDamage)}</td><td>${num(a.flankHits)}</td><td>${rate(a.flankHits,a.hullHits)}</td><td>${num(a.flankDamage)}</td><td>${a.hullHits?num(a.maxHit):'—'}</td></tr>`).join('');
}

function renderBriefing(player, encounter) {
 const pct=player.total?100*player.pets/player.total:0;
 $('briefing-context').textContent=`${player.name} · ${encounter.stamp} · ${encounter.duration.toFixed(1)}s · ${encounter.scope==='full-log'?'Entire log':encounter.targets.join(', ')||'Individual encounter'}`;
 $('metrics').innerHTML=metric(encounter.scope==='full-log'?'FULL-LOG DPS':'ENCOUNTER DPS',num(player.dps),'Hull + shield damage')+metric('TOTAL DAMAGE',compact(player.total),`${compact(player.hull)} hull / ${compact(player.shield)} shield`)+metric('PET CONTRIBUTION',`${pct.toFixed(1)}%`,`${compact(player.pets)} damage from owned pets`);
 const contributors=[...player.abilities].sort((a,b)=>b.total-a.total).slice(0,3);
 $('briefing-contributors').innerHTML=contributors.map(a=>`<div class="briefing-contributor"><div>${esc(a.name)}<small>${esc(a.source)}</small></div><div class="briefing-track"><i style="width:${contributors[0]?.total?100*a.total/contributors[0].total:0}%"></i></div><strong>${compact(a.total)}</strong></div>`).join('')||'<p>No outgoing damage recorded.</p>';
 $('briefing-mix').textContent=`Pets supplied ${pct.toFixed(1)}% of this run’s outgoing damage.`;
 const pets=new Map();for(const a of player.abilities.filter(a=>a.pet))pets.set(a.source,(pets.get(a.source)||0)+a.total);
 const best=[...pets].sort((a,b)=>b[1]-a[1])[0];
