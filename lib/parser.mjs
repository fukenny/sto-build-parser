...
export function createParser(gapSeconds = 60) {
  ...
  function finish() {
    ...
    for (const p of players) {
      p.survival = finishSurvival(p.survival);
      p.abilities = p.abilities.map(a=>({...a,targets:[...a.targets.values()].sort((x,y)=>y.total-x.total)}));
    }
  ...
  return {
    add(line) {
      ...
      if (!p.abilities.has(key)) p.abilities.set(key, {name: e.ability, source: pet ? e.source : 'Ship / player', pet, targets:new Map(), total: 0, hull: 0, shield: 0, events: 0, hullHits: 0, criticalHits: 0, misses: 0, flankHits: 0, criticalDamage: 0, flankDamage: 0, maxHit: 0});
      const a = p.abilities.get(key);
      limit(p.abilities.size,2000,'ability count');
      // Group identical recorded target names; do not imply individual enemy identity.
      const targetName=e.target || 'Unknown target';
      if(!a.targets.has(targetName)) {
        limit(a.targets.size+1,2000,'ability target count');
        a.targets.set(targetName,{name:targetName,total:0,hull:0,shield:0,events:0,hullHits:0,misses:0,criticalHits:0,flankHits:0,criticalDamage:0,flankDamage:0,maxHit:0});
      }
      for(const row of [a,a.targets.get(targetName)]) {
      row.total += e.amount; row[e.type === 'Shield' ? 'shield' : 'hull'] += e.amount; row.events++;
      // Count non-shield records so a hull/shield pair is not two hit attempts.
      if (e.type !== 'Shield') {
        if (e.flags.includes('Miss')) row.misses++;
        else {
          row.hullHits++;
          row.maxHit = Math.max(row.maxHit,e.amount);
          if (e.flags.includes('Critical')) { row.criticalHits++; row.criticalDamage+=e.amount; }
          if (e.flags.includes('Flank')) { row.flankHits++; row.flankDamage+=e.amount; }
        }
      }
      }
      const second = Math.max(0, Math.floor((e.time - current.start) / 1000));
