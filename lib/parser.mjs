...
export function createParser(gapSeconds = 60) {
  ...
  return {
    add(line) {
      ...
      if (!p.abilities.has(key)) p.abilities.set(key, {name: e.ability, source: pet ? e.source : 'Ship / player', pet, total: 0, hull: 0, shield: 0, events: 0, hullHits: 0, criticalHits: 0, misses: 0, flankHits: 0, criticalDamage: 0, flankDamage: 0, maxHit: 0});
      ...
      if (e.type !== 'Shield') {
        if (e.flags.includes('Miss')) a.misses++;
        else {
          a.hullHits++;
          a.maxHit = Math.max(a.maxHit,e.amount);
          if (e.flags.includes('Critical')) { a.criticalHits++; a.criticalDamage+=e.amount; }
          if (e.flags.includes('Flank')) { a.flankHits++; a.flankDamage+=e.amount; }
        }
      }
      const second = Math.max(0, Math.floor((e.time - current.start) / 1000));
